            ...
            <div class="card-body">
                ...
                <table class="table" id="agents">
                    ...
                    <tbody>
                        @if ($debit->isEmpty())
                            <tr>
                                <td colspan="5" class="text-center text-danger">No Debit Entries Added Yet</td>
                            </tr>
                        @else
                            @foreach ($debit as $debt)
                                <tr>
                                    <td class="text-center">{{ $debt->date }}</td>
                                    {{--  <td class="text-center">{{ $debt->boardMember->name }}</td>  --}}
                                    <td class="text-center">{{ $debt->boardMember->name ?? 'N/A' }}</td>
                                    <td class="text-center">₹{{ $debt->amount }}</td>
                                    <td class="text-center">{{ $debt->remark }}</td>
                                    <td style=" display:flex; justify-content:center;align-items-center">
                                        <a href="#" class="text-info mr-3 edit_agent"
                                            data-debt-id="{{ $debt->id }}" data-toggle="modal"
                                            data-target="#exampleModal"><span class="mdi mdi-pencil text-center"></span></a>
                                        <form method="POST" action="{{ route('debit.destroy', $debt->id) }}"
                                            class="inner">
                                            @csrf
                                            @method('DELETE')
        ...
        <div class="modal-dialog " role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Add Debit Entry</h5>
                </div>
                <div class="modal-body">
                    <form id="agentForm" method="post" action="">
                        ...
                        <div class="modal-footer">
                            <button type="submit" class="btn btn-primary btn-sm" id="saveButton">Add debts</button>
    ...
    <script>
        $(document).ready(function() {
            $('.edit_agent').click(function() {
                ...
                $.ajax({
                    url: '/debit/' + debtId + '/edit',
                    type: 'GET',
                    success: function(response) {
                        //console.log(response.email);

                        $('#exampleModalLabel').text('Edit Debit Entry');
                        $('#saveButton').text('Update Debit Entry');
                        // Update the form action to include the agent ID
                        $('#agentForm').attr('action', '/debit/' + debtId);
                        $('#debtId').val(debtId);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            icon: 'success',
                            title: 'Success',
                            text: 'Debit Entry ' + (method == 'POST' ? 'added' :
                                    'updated') +
                                ' successfully',
                        }).then((result) => {
                            location.reload();
